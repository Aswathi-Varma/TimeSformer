...
import numpy as np
from dataset.dataset_utils_2D import retrieve_data_dir_paths, Modalities, Phase, Views, Mode
...
class DatasetSeg2D(Dataset):
    ...
    def __init__(self, data_dir, datalist, batch_size, phase=Phase.TRAIN, modalities=(), dim=None, transforms = None, preprocess=True,
        ...
        self.dim = dim
        self.modalities = ['FLAIR', 'T2']
        self.return_mask = True

        self.use_z_score = True
        self.transforms = transforms
        self.batch_size = batch_size
        self.data_dir_paths = retrieve_data_dir_paths(data_dir, datalist, phase, preprocess, self.mode, view)
    ...
    def __getitem__(self, idx):
        ...
        if self.return_mask:
            ...
            norm_x = self._normalize_data(x.clone())
            return norm_x.float(), label_1.float(), label_2.float()
        else:
            ...
            norm_x = self._normalize_data(x.clone())
            return norm_x.float(), label_2.float()
