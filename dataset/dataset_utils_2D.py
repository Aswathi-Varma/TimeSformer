...
def retrieve_data_dir_paths(data_dir, datalist, phase, preprocess, mode, view=None):
    empty_slices = None
    non_positive_slices = None
    if preprocess:
        print('Preprocessing files...')
        empty_slices, non_positive_slices = preprocess_files(data_dir, datalist, phase)
...
def preprocess_files(root_dir, datalist, phase, base_path='data'):
    patients = datalist
    empty_slices = []
    non_positive_slices = []
    ...
    for patient in patients:
        ...
        for modality in list(Modalities):
            ...
            for timestep in timepoints:
                data_path = f'{patient_path}/{timestep}/{patient}_{timestep}_{mod}.nii.gz'
                ...
                if os.path.exists(label_path):
                    rotated_labels = pad_to_largest_dimension(label_path)
                else:
...
def pad_to_largest_dimension(data_path):
    ...
    # save as nifti
    save_nifti(data, nib.load(data_path).affine, 'check.nii.gz')
