...
import math
from model.TSFormerAttention import TSFormerAttention
...
class STLayer(nn.Module):
    def __init__(self, dim, mlp_ratio=4, drop=0., drop_path=0., act_layer=nn.GELU):
        ...
        self.vit = TSFormerAttention(img_size=224, num_frames=2, embed_dim=dim)
    ...
    def forward(self, x):
        ...
        x_st = (x + self.drop_path(self.vit(self.norm1(x)))).reshape(B, nf*H*W, C)
        x_st = x_st + self.drop_path(self.mlp(self.norm2(x_st), nf, H, W))
        out = x_st.transpose(-1, -2).reshape(B, nf, N, C)
...
class st_block(nn.Module):
    ...
    """
    def __init__(self, backbone, in_chans=1, depths=[2, 2, 2, 2], dims=[32, 64, 160, 256],
    ...
    def forward_features(self, x):
        ...
        for idx, x in enumerate(zip(self.downsample_layers.patch_embeddings, self.downsample_layers.block, self.downsample_layers.layer_norm, self.stages)):
            embedding_layer, block_layer, norm_layer, st_stage = x
            ...
            hs = norm_layer(hs)
            hs = hs.reshape(bz, nf, height * width, -1)
            hs = st_stage(hs).reshape(bz, -1, nf, height, width)
            # print(x.shape)
            hs = hs.transpose(1,2)
            hs = hs.reshape(bz*nf,hs.shape[-3],hs.shape[-2],hs.shape[-1])
           # hs = hs.reshape(bz * nf, -1, height, width)
...
class SegFormerTS(nn.Module):
    def __init__(
        self,
        in_chans=3,
        num_classes=1,
        depths=[2, 2, 2, 2],
        feat_size=[32, 64, 160, 256],
        drop_path_rate=0,
        ...
        backbone.segformer.encoder.patch_embeddings[0].proj = nn.Conv2d(
            2, 32, kernel_size=(7, 7), stride=(4, 4), padding=(3, 3)
        )

        # Define a custom Segformer configuration for Segformer-b3 architecture
        # config = SegformerConfig(
        #     num_channels=in_chans,
        #     hidden_sizes=[64, 128, 320, 512],
        #     num_attention_heads=[1, 2, 5, 8],
        #     depths=[3, 4, 18, 3],
        #     num_labels=1,  # Set based on your number of classes
        #     image_size=224,  # Adjust as needed
        # )

        # # Initialize the Segformer model with this custom config
        # backbone = SegformerForSemanticSegmentation(config)


        self.encoder = st_block(backbone, in_chans, dims=feat_size,
                              )
        self.decoder = backbone.decode_head

        self.out = nn.Conv2d(256, num_classes, kernel_size=1)
    ...
    def forward(self, x_in):
        ...
        upsampled_logits = nn.functional.interpolate(
            ...
            ).reshape(bz, nf, self.out_chans, h, w)
