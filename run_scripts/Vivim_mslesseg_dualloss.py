...
import tqdm
sys.path.append("/root/seg_framework/ST-Seg")
import numpy as np
import torch
...
#from model.SegFormerST import SegFormerST
from model.TimeSformer import TimeSformer
from utils.metric import precision, recall, dice_score    
from utils.vivim_loss import dual_loss
import wandb
sys.stdout = open("/root/seg_framework/ST-Seg/sys_out", "w")
...
def get_args_parser():
    ...
    parser.add_argument('--loss', default='tripleloss', type=str,
                        help='resume from checkpoint')
...
def main(args):
    ...
    datafolders = np.load('/root/seg_framework/ST-Seg/dataset/patients_mslesseg.npy')

    # Code for the K-fold cross validation
    kfold_splits = KFold(n_splits=5, random_state=1800, shuffle=True)

    # Create the location for storing split indices
    split_index_path = os.path.join(PROJECT_ROOT_DIR, "mslesseg", 'k_fold', 'indices_file')
    ...
    for idx in range (1):
        ...
        #Get the train, val and test set
        ...
        #Prepare data
        train_dataset = DatasetSeg2D(data_dir=args.data_dir, datalist=train_set, batch_size=args.batch_size, 
                                phase='train', dim=args.dim, transforms=None, preprocess=args.preprocess)
        val_dataset = DatasetSeg2D(data_dir=args.data_dir, datalist=val_set, batch_size=args.batch_size, 
                                phase='val', dim=args.dim, transforms=None, preprocess=args.preprocess)
        test_dataset = DatasetSeg2D(data_dir=args.data_dir, datalist=test_set, batch_size=args.batch_size,
                                    phase='test', dim=args.dim, transforms=None, preprocess=args.preprocess)
        ...
        if args.log_dir is not None and not args.eval:
                log_dir = os.path.join(PROJECT_ROOT_DIR, args.log_dir)
                os.makedirs(log_dir, exist_ok=True)
                log_writer_train = SummaryWriter(log_dir=f"{log_dir}/train_ft")
                log_writer_val = SummaryWriter(log_dir=f"{log_dir}/val_ft")
        ...
        max_prec, max_recall, max_dice = 0, 0, 0
        ...
        for epoch in range(args.start_epoch, args.epochs):
                ...
                print(f"Dice score of the network on the train images: {train_val_stats['dice']:.6f}, val images: {val_stats['dice']:.6f}")
                max_prec = select_best_model(args=args, epoch=epoch, loss_scaler=loss_scaler,
                                                    max_val=max_prec,
                                                    model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                                                    cur_val=val_stats['prec'],
                                                    model_name=f'best_prec_model_{idx}', output_dir=output_dir)

                max_recall = select_best_model(args=args, epoch=epoch, loss_scaler=loss_scaler, max_val=max_recall,
                                            model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                                            cur_val=val_stats['rec'], model_name=f'best_rec_model_{idx}', output_dir=output_dir)
                max_dice = select_best_model(args=args, epoch=epoch, loss_scaler=loss_scaler, max_val=max_dice,
                                            model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                                            cur_val=val_stats['dice'], model_name=f'best_dice_model_{idx}', output_dir=output_dir)  
            
                # Writing the logs
                log_writer_val.add_scalar('ft/precision', val_stats['prec'], epoch)
                log_writer_val.add_scalar('ft/loss', val_stats['loss'], epoch)
                log_writer_val.add_scalar('ft/recall', val_stats['rec'], epoch)
                log_writer_val.add_scalar('ft/dice', val_stats['dice'], epoch)
                log_writer_train.add_scalar('ft/precision', train_val_stats['prec'], epoch)
                log_writer_train.add_scalar('ft/loss', train_val_stats['loss'], epoch)
                log_writer_train.add_scalar('ft/recall', train_val_stats['rec'], epoch)
                log_writer_train.add_scalar('ft/dice', train_val_stats['dice'], epoch)                                
        ...
        evaluate_test(data_loader=dataloader_test, model=model, device=device)
